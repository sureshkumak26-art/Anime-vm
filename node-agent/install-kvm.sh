...
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y qemu-kvm libvirt-daemon-system libvirt-clients qemu-utils cloud-image-utils bridge-utils curl ca-certificates jq
systemctl enable --now libvirtd
mkdir -p /opt/anime-cloud-agent /var/lib/anime-cloud/images /var/lib/anime-cloud/vms

cat >/etc/anime-cloud-agent.env <<'ENVEOF'
...
cat >/etc/systemd/system/anime-cloud-agent.service <<'UNITEOF'
[Unit]
...
cat >/opt/anime-cloud-agent/agent.sh <<'AGENTEOF'
#!/usr/bin/env bash
set -euo pipefail
...
echo 'Edit /etc/anime-cloud-agent.env with ANIME_CLOUD_API, NODE_ID and AGENT_KEY, then restart anime-cloud-agent.'
